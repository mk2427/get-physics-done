...
Branch `experimental/knowledge-trust-full` carries the **Knowledge Trust & Blast Radius** feature set (brief 002). This surface is experimental — it is developed on a PSI fork and has not been merged to `psi-oss` upstream.

**Components**: knowledge-doc adversarial digest, `/gpd:adversarial-review` primitive, assertion doc lifecycle, EQN-REF integrator, blast-radius invalidation graph, `/gpd:new-project` ingest hooks.

**Status**: Implementation complete (commits 1–12). BFSS 16-PDF canary scheduled. See `docs/experimental/knowledge-trust-full/README.md` for full details.

**Fork posture**: Fork-only per brief §3.6. CLA is on file (non-blocking). Do not submit these files to upstream without explicit CLA + maintainer agreement.
